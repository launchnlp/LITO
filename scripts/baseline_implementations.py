...
def get_base_results():
    ...
    acc = base_df[column_name].mean()
    f1 = 2 * (acc * 1) / (acc + 1)
    f1 = round(f1*100, 2)
    print(f"no intervention: Acc: {round(acc*100, 2)}, F1:{f1}")

    df = pd.read_csv(data_file)
    for i in range(1 , 6): 
        column_name = f"label_{i*5}"
        acc = df[column_name].mean()
        f1 = 2 * (acc * 1) / (acc + 1)
        f1 = round(f1*100, 2)
        print(f"alpha {i*5}: Acc: {round(acc*100, 2)}, F1:{f1}")
...
def majority_vote():
    ...
    print(f"majority vote: Acc: {round(final_label/len(dataset)*100, 2)}, P/R/F1: {tuple([p,r,f1])}")
...
def max_confidence():
    ...
    p,r,f1 = f1_calc(tp, fp, fn)
    p,r,f1 = round(p*100, 2), round(r*100, 2), round(f1*100, 2)
    print(f"maximum confidence: Acc: {round(final_label/len(dataset)*100, 2)}, P/R/F1: {tuple([p,r,f1])}")
...
def max_confidence_greater_than_t(threshold=0.6):
    ...
    p,r,f1 = f1_calc(tp, fp, fn)
    p,r,f1 = round(p*100, 2), round(r*100, 2), round(f1*100, 2)
    print(f"maximum confidence > {threshold}: Acc: {round(final_label/len(dataset)*100, 2)}, P/R/F1: {tuple([p,r,f1])}")
    print(f"maximum confidence > {threshold}: Truthfulness: {round(100*truth_label/len(dataset), 2)}")  
    print(f"maximum confidence > {threshold}: Accuracy (Factuality): {round(100*tp/len(dataset),2)}")
...
if __name__ == "__main__":
    ...
    max_confidence()
    print('\n')
    max_confidence_greater_than_t(threshold=0.6) # change this
    print('\n')
    max_confidence_greater_than_t(threshold=0.7) # change this
    print('\n')
    max_confidence_greater_than_t(threshold=0.8) # change this
